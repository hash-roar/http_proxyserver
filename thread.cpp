...
void server_thread::Main()
{
    ...
    while (!is_exit())
    {
        ...
        {
            logEvent log_event(http_obj.head_line+"非GET方法访问  50X  BADGETWAY  "+http_obj.get_http_info("User-Agent"), WARNING, 0, getpid());
        ...
        // 写入日志
        logEvent log_event(http_obj.head_line+"  200  OK  "+http_obj.get_http_info("User-Agent"),INFO);
